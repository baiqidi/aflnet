...
5. **排序开关**：使用 `-O on|off` 控制是否启用覆盖调度器的种子排序；
   `-O on`（默认）启用新颖度排序，`-O off` 则完全沿用 AFLNet 原有的队列遍历。
6. **聚类模式**：运行时通过 `-G` 选择候选分类策略：
   * `-G state`（默认）按状态集合去重。
...
2. **运行 AFLNet**：依据协议类型设置 `-N`（传输层）和 `-P`（协议名称），再加上
   ...
   AFL_DEBUG_OVERLAY=1 AFL_STAT_OVERLAY=1 ./afl-fuzz \
     ...
     -N udp -P XYZ -D 1000 -O on -G state -- ./target_binary @@
   ```

   * 如果需要 TLS、TCP 或自定义端口，请参考 `README.md` 调整附加参数。
   * 运行后在终端中观察 `overlay: cluster=... novelty=...` 等日志，确认覆盖调度器
     已经接管候选排序流程。不同的 `-G` 模式会在日志的 `mode=` 字段中体现；若使用
     `-O off` 则不会输出这些调度日志，调度逻辑也会回退到 AFLNet 原实现。
3. **监控运行状态**：
