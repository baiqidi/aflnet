...
6. **How do I change the overlay clustering strategy or disable sorting?** Pass `-G state`, `-G none`, or `-G shingle` when launching `afl-fuzz` to choose between state-set deduplication, flat ordering, or k=3 shingle clustering. Use `-O off` to bypass the overlay scheduler entirely and fall back to AFLNet’s legacy queue traversal, or `-O on` to re-enable novelty-based ordering.
...
- **Overlay scheduler smoke run (replace placeholders):**
  ...
  AFL_DEBUG_OVERLAY=1 ./afl-fuzz \
    ...
    -N tcp://127.0.0.1/<port> -P <protocol_name> -O on -G state [extra AFLNet flags] -- \
    ./path/to/server_binary <server_args>
  ```
- Add `-O off` to revert to original AFLNet scheduling without novelty sorting.
